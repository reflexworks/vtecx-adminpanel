...
export const addUserMailAtom = atom<VtecxApp.Entry>()
...
const fetchListAtom = atom(null, async (get, set) => {
  ...
  const newPromise = (async () => {
    try {
      const listRes = (await fetcher(`/d/_settings/adduser?e`, 'get')) || {}
      set(addUserMailAtom, listRes?.data || {})
...
const useAddUserMail = () => {
  const { setLoader } = useLoader()

  const { setError: setGeneralError } = useGeneralError()

  const [data] = useAtom(addUserMailAtom)
